...
<head>
    <meta charset="utf-8">
    <title>Edit Import Receipt</title>
...
<div class="container-fluid pt-4 px-4">
    <div class="bg-light rounded p-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h6 class="mb-0">Sửa phiếu nhập #${receipt.receiptId}</h6>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" name="supplierId">
                        ...
                        <c:forEach var="s" items="${suppliers}">
                            <option value="${s.supplierId}" ${s.supplierId == receipt.supplierId ? 'selected' : ''}>${s.name}</option>
                        </c:forEach>
                    </select>
                </div>
                <div class="col-md-8">
                    <label class="form-label">Ghi chú</label>
                    <input class="form-control" name="note" value="${receipt.note}"/>
                </div>
            </div>
                            ...
                            <td>
                                <select class="form-select" name="bookId">
                                    ...
                                    <c:forEach var="b" items="${books}">
                                        <option value="${b.bookId}" ${b.bookId == d.bookId ? 'selected' : ''}>#${b.bookId} - ${b.title}</option>
                                    </c:forEach>
                                </select>
                            </td>
                            <td><input class="form-control" name="qty" type="number" min="1" value="${d.qty}"/></td>
                            <td><input class="form-control" name="importPrice" type="number" step="0.01" min="0" value="${d.importPrice}"/></td>
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
